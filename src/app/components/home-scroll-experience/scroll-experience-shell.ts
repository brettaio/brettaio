...
import {
  ScrollExperienceAction,
  ScrollExperienceConfig,
  ScrollExperienceFrame,
} from './scroll-experience.types';
...
@Component({
  selector: 'bretta-scroll-experience-shell',
  imports: [RouterLink],
  template: `
    <section
      #heroSection
...
export class ScrollExperienceShell implements AfterViewInit {
  ...
  readonly config = input.required<ScrollExperienceConfig>();
  ...
  protected readonly desktopEmailAction = computed<ScrollExperienceAction>(() => ({
    label: this.config().primaryCtaLabel,
    href: this.resolveEmailHref(),
    variant: 'solid',
  }));

  protected readonly continueAction = computed<ScrollExperienceAction>(() => ({
    label: this.config().secondaryCtaLabel,
    href: this.config().secondaryCtaHref,
    variant: 'outline',
  }));

  protected readonly inlineActions = computed<ScrollExperienceAction[]>(() => {
    if (this.isMobile()) {
  ...
  protected readonly frames = computed<ScrollExperienceFrame[]>(() => {
    const frames = this.config().frames;
    const actions = this.inlineActions();

    return frames.map((frame, index) => {
      if (index !== frames.length - 1 || frame.actions?.length) {
        return frame;
      }
  ...
  protected readonly frameWeights = computed<number[]>(() => this.config().frameWeights);
  ...
  protected actionClass(action: ScrollExperienceAction): string {
    const base =
  ...
  protected handleActionClick(value: string): void {
    if (value.startsWith('mailto:')) {
      this.analytics.trackEmailClick(this.config().id);
      return;
    }
  ...
  protected resolveEmailHref(): string {
    return this.config().primaryCtaHref || this.buildDesktopMailtoHref();
  }

  private buildDesktopMailtoHref(): string {
    const subject = encodeURIComponent('Conversation — bretta.io');
    const body = encodeURIComponent(
      [
        'Hi Brett,',
        '',
        "I'm reaching out about connection, belonging, systems, or the work you are building.",
        '',
        'Name:',
        'Context:',
  ...
  private startShaderScene(): void {
    ...
    const fragmentShader = this.createShader(
      gl,
      gl.FRAGMENT_SHADER,
      this.config().fragmentShader || DEFAULT_FRAGMENT_SHADER
    );
  ...
  private updateSceneFromScroll(): void {
    ...
    this.shaderTime = buildShaderTimeFromScroll(
      progress,
      this.config().scrollTimeRange
    );
  }
