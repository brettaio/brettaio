      ...
      <div class="mx-auto max-w-6xl space-y-10">
        <div class="max-w-3xl">
          ...
          <p class="mt-6 max-w-2xl text-lg leading-8 text-white/80">
            Keep it simple. Leave your details and I’ll get back to you.
          </p>
        </div>
        ...
        >
          ...
          <div class="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-8">
            <p class="max-w-xl text-sm leading-6 text-white/60">
              A simple starting point. Leave the basics and I’ll reply directly.
            </p>
        ...
        <div
          class="rounded-3xl border border-white/10 bg-[#1b0b2d]/60 p-6 shadow-2xl shadow-black/20 backdrop-blur-sm sm:p-8 lg:p-10"
        >
          <div class="max-w-3xl">
            <p class="text-sm font-medium uppercase tracking-[0.28em] text-white/70">
              Project inquiry
          ...
          @if (!showProjectInquiry()) {
            <div
              class="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-8"
            >
              <p class="max-w-xl text-sm leading-6 text-white/60">
                Need a sharper intake? Open the full inquiry and I’ll get the
                commercial context I need up front.
              </p>

              <button
                type="button"
                (click)="revealProjectInquiry(true)"
                class="inline-flex items-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-black transition hover:bg-white/90"
              >
                Let's continue
              </button>
...
export class LeadForm implements OnInit {
  ...
  protected readonly showProjectInquiry = signal(false);
  ...
  ngOnInit(): void {
    ...
    const onOpenProjectInquiry = (): void => {
      this.showProjectInquiry.set(true);
    };

    window.addEventListener(
      'bretta:open-project-inquiry',
      onOpenProjectInquiry as EventListener
    );
  ...
  protected revealProjectInquiry(scrollIntoView = false): void {
    this.showProjectInquiry.set(true);

    if (!scrollIntoView || !isPlatformBrowser(this.platformId)) {
      return;
    }
  ...
  protected onConversationStarterSubmit(): void {
    this.analytics.trackLeadGenerated('conversation-starter');
  }

  protected onProjectInquirySubmit(): void {
